...
public class HouseholdsPanel extends AbstractChartsPanel implements
	...
	public class HouseholdsPanelForCurrency extends JPanel implements
		...
		public synchronized void notifyListener() {
			if (this.isShowing()) {
				...
				SummaryStatisticalData summaryStatisticalData = ApplicationContext
	...
	protected ChartPanel createIncomeSourcePanel(Currency currency) {
		...
		for (IncomeSource incomeSource : ApplicationContext.getInstance()
	...
	protected JFreeChart createIncomeDistributionPanel(Currency currency) {
		IntervalXYDataset dataset = ApplicationContext.getInstance()
				.getModelRegistry().getNationalEconomyModel(currency).householdsModel.incomeDistributionModel
				.getHistogramDataset();
